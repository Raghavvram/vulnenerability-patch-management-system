...
class GroqLLMAnalyzer:
    ...
    async def call_groq_api(self, system_prompt: str, user_prompt: str) -> str:
...
class LLMAnalyzer:
    def __init__(self):
        self.groq_analyzer = GroqLLMAnalyzer()
        self.perplexity_analyzer = PerplexityLLMAnalyzer()
...
- name: Emergency patch for {service_name}
  ...
  tasks:
    ...
    async def analyze_vulnerability(self, vulnerability_data: Dict) -> VulnerabilityAnalysis:
        ...
        # Try Groq first
        if self.groq_analyzer.api_key:
            try:
                logger.info("Attempting analysis with Groq")
                response_content = await self.groq_analyzer.call_groq_api(system_prompt, user_prompt)
                ...
                logger.warning(f"Groq analysis failed: {e}. Falling back to Perplexity.")

        # Fallback to Perplexity
        if self.perplexity_analyzer.api_key:
            try:
                logger.info("Attempting analysis with Perplexity")
                response_content = await self.perplexity_analyzer.call_perplexity_api(system_prompt, user_prompt)
                cleaned_response = response_content.strip()
                if cleaned_response.startswith("```json"):
                    cleaned_response = cleaned_response[7:]
                if cleaned_response.startswith("```"):
                ...
                logger.warning(f"Perplexity analysis failed: {e}. Falling back to rule-based analysis.")

        # Final fallback
        logger.warning("All LLM providers failed, using fallback analysis")
...
async def analyze_enriched_hosts(enriched_hosts: List[Dict]) -> Dict:
    ...
    return {
        ...
        "summary": {
            ...
            "llm_provider": "groq_compound/perplexity_sonar_fallback",
            "model": analyzer.groq_analyzer.model
        }
    }